...
const Columns = ({addTask , updateTask, setActiveCol, onDropCol, deleteTask, setActiveTask,  onDropTask,  dataTask, dataColumn, board, deleteColumn,  updateColumn, darkMode}: ColumnsForms) => {
    ...
    return (
        <>
            {dataColumn.length >= 1 && dataColumn.map((column, index) => (
                column.boardId === board?.id && (
                    <div key={column.id} className='flex'>
                        <div draggable onDragStart={() => setActiveCol(index)} onDragEnd={() => setActiveCol(null)} 
                            className={` w-[300px] relative  mb-2  max-h-[75vh] flex-none  transition-all duration-500 ${darkMode ? 'bg-[#0d0d0eb6] ' :
                            ...
                            {columnId !== column.id ? (
                                ...
                                <form onSubmit={(e) => {handleChangeColumnName(e)}} className=" items-center justify-between flex gap-1 pr-2">
                            ...
                            <DropAreaCols darkMode={darkMode} onDropCol={() => onDropCol(index)}></DropAreaCols>
